let
  ...
  # };
  nix-bitcoin-path = ../.;
  nix-bitcoin = (import <nixpkgs> {}).callPackage (nix-bitcoin-path) {};
  nixpkgs = (import nix-bitcoin.nixpkgs-pinned).nixpkgs;
in
with import nixpkgs { };
...
stdenv.mkDerivation rec {
  ...
  shellHook = ''
  export NIX_PATH="nixpkgs=${nixpkgs}:nix-bitcoin=${toString nix-bitcoin-path}:."
